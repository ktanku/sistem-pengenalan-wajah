    ...
    def TakePhoto(self):
        ...
        def create_folder(name):
            ...
            if os.path.exists(name):
                checkstring = "Maaf, Username Sudah Ada, Coba Lagi ... !!!!"     
                close_button = MDFlatButton(text="Keluar",on_release= self.close)
                self.dialog = MDDialog(title="Proses Check Username",text=checkstring,
                                        size_hint = (0.5,1),
                ...
                checkstring = "Silahkan Input Username Anda"
                close_button = MDFlatButton(text="Keluar",on_release=self.close)
                self.dialog = MDDialog(title="Proses Check Username",text=checkstring,
                                        size_hint = (0.5,1),
                                        buttons = [close_button]  )
                ...
                self.flag = 1
                os.mkdir(name)
                checkstring = "Silahkan Melanjut ke Proses Pelatihan."
                close_button = MDFlatButton(text="Keluar",on_release=self.close)
                self.dialog = MDDialog(title=f"Selamat, {name} Anda Sukses Terdaftar",text=checkstring,
                                        size_hint = (0.5,1),
                                        buttons = [close_button]  )
                self.dialog.open()
        ...
        if name.isalpha() and self.flag == 1:
            ...
            while True:
                ...
                if face_extractor(frame) is not None:
                    ...
                    cv2.putText(face,f"Sabar Menunggu {count}sec.",(6,55),cv2.FONT_HERSHEY_COMPLEX,0.5,(255,255,255),2)
                    cv2.imshow('Cropping Area Wajah', face)

                else:
                    #print("Face not found")
                    pass

                if cv2.waitKey(1) == 27 or count==1:
                    break
...
class TrainPhotoScreen(Screen):
    ...
    def TrainData(self):
        ...
        #image_dir = os.path.join(BASE_DIR, "data_wajah")
        ...
        for root, dirs, files in os.walk(os.path.join(image_dir)):
            #print(root)
            for file in files:
                #print(file)
                if file.endswith('png') or file.endswith('jpg'):
                    ...
                    label = os.path.basename(img_path).replace(" ", "-").capitalize()
        ...
        checkstring = "Perangkat Anda Telah Siap Melakukan Deteksi Wajah!"
        close_button = MDFlatButton(text="Keluar",on_release=self.close)
        self.dialog = MDDialog(title="Pelatihan Model Dataset!!",text=checkstring,
                                size_hint = (0.5,1),
                                buttons = [close_button]  )
        self.dialog.open()
...
class MainApp(MDApp):
    ...
    def change_profile_source(self, path):
        self.root.ids.profile.source = "/home/ijtraigan/"+path # For computer
        ...
        with open("profile_source.txt", "w") as f:
            ...
            f.write("/home/ijtraigan/"+path) # For computer
    ...
    def change_screen(self, screen_name, direction='forward', mode = ""):
        ...
        if screen_name == "home_screen":
            #print(screen_name)
            self.root.ids.titlename.title = "Deteksi Wajah"
        
        if screen_name == "profilephoto_screen":
            self.root.ids.titlename.title = "Ubah Profile Disini"
        ...
        if screen_name == "takephoto_screen":
            self.root.ids.titlename.title = "Ambil Foto Anda"
            #print("Screen name is ", screen_name)

        if screen_name == "trainphoto_screen":
            self.root.ids.titlename.title = "Train Foto Anda"
            #print("Screen name is ", screen_name)
